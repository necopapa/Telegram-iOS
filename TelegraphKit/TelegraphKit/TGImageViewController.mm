...
- (void)viewDidAppear:(BOOL)animated
{
    if (!_alreadyAnimatedAppearance)
    {
        ...
        {
            _backgroundView.hidden = true;
            [self animateAppear:_appearFromView anchorForImage:_appearAboveView fromRect:_appearFromRect fromImage:_fromImage start:_appearStart];
...
- (void)animateDisappear:(UIView *)containerForImage anchorForImage:(UIView *)anchor toRect:(CGRect)rectInWindowSpace toImage:(UIImage *)toImage swipeVelocity:(float)__unused swipeVelocity completion:(dispatch_block_t)completion
{
    [_interfaceView setActive:false duration:0.3 statusBar:false];
    ...
    [page animateDisappearToImage:nil toView:containerForImage aboveView:anchor toRect:rectInWindowSpace toContainerImage:toImage toInterfaceOrientation:self.interfaceOrientation keepAspect:_keepAspect backgroundAlpha:_backgroundView.alpha swipeVelocity:_disappearSwipeVelocity completion:completion];
}
...
- (void)actionStageActionRequested:(NSString *)action options:(NSDictionary *)__unused options
{
    if ([action isEqualToString:@"animateDisappear"])
    {
        ...
        if (watcher != nil && [watcher respondsToSelector:@selector(actionStageActionRequested:options:)])
        {
            ...
            NSMutableDictionary *closeDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:self, @"sender", [NSNumber numberWithDouble:0.3], @"duration", nil];
            if (imageInfo != nil)
...
- (void)saveToCameraRoll:(NSData *)data
{
    ...
    [assetsLibrary writeImageDataToSavedPhotosAlbum:data metadata:nil completionBlock:^(NSURL *assetURL, NSError *error)
    {
        if (error != nil)
        {
            dispatch_async(dispatch_get_main_queue(), ^
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"An error occured" delegate:nil cancelButtonTitle:NSLocalizedString(@"Common.Cancel", nil) otherButtonTitles:nil];
                [alertView show];
            });
        }
        else
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
        if (_interfaceView.topPanelView.alpha > FLT_EPSILON)
            [_interfaceView setActive:false duration:0.3];
    }
}
