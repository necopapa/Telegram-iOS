...
@synthesize fadeViewLeft = _fadeViewLeft;
@synthesize fadeViewRight = _fadeViewRight;
@synthesize fadeViewTop = _fadeViewTop;
@synthesize fadeViewBottom = _fadeViewBottom;

@synthesize targetImageContainer = _targetImageContainer;
@synthesize targetImageBackgroundView = _targetImageBackgroundView;
...
- (void)beginTransitionIn:(UIView *)imageView fromImage:(UIImage *)fromImage fromView:(UIView *)fromView fromRectInWindowSpace:(CGRect)fromRectInWindowSpace aboveView:(UIView *)aboveView toView:(UIView *)toView toRectInWindowSpace:(CGRect)toRectInWindowSpace toInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation completion:(dispatch_block_t)completion keepAspect:(bool)keepAspect
{
...
- (void)beginTransitionOut:(UIView *)imageView fromView:(UIView *)fromView toView:(UIView *)toView aboveView:(UIView *)aboveView interfaceOrientation:(UIInterfaceOrientation)interfaceOrientation toRectInWindowSpace:(CGRect)toRectInWindowSpace toImage:(UIImage *)toImage keepAspect:(bool)keepAspect swipeVelocity:(float)swipeVelocity completion:(dispatch_block_t)completion
{
    ...
    [UIView animateWithDuration:(CGSizeEqualToSize(targetFrame.size, CGSizeZero) ? 0.3 : 0.2) delay:(CGSizeEqualToSize(targetFrame.size, CGSizeZero) ? 0.0 : 0.1) options:0 animations:^
    {
        imageView.alpha = 0.0f;
    } completion:nil];
    ...
    [UIView animateWithDuration:0.3 animations:^
    {
        ...
        {
            float offset = MAX(ABS(swipeVelocity * 0.3f), screenSize.height);
