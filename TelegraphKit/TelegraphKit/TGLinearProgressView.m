...
@property (nonatomic) float minProgressWidth;
...
@synthesize alwaysShowMinimum = _alwaysShowMinimum;

@synthesize backgroundView = _backgroundView;
@synthesize progressView = _progressView;

@synthesize minProgressWidth = _minProgressWidth;
...
- (void)setProgress:(float)progress animationDuration:(NSTimeInterval)animationDuration
{
    ...
    float progressWidth = (frame.size.width - 2) * _progress;
    
    CGRect progressFrame = CGRectMake(1, 1, _alwaysShowMinimum ? MAX(progressWidth, _minProgressWidth) : progressWidth, _progressView.frame.size.height);
...
- (void)setFrame:(CGRect)frame
{
    ...
    CGRect progressFrame = CGRectMake(1, 1, (frame.size.width - 2) * _progress, _progressView.frame.size.height);
    _progressView.alpha = progressFrame.size.width < _minProgressWidth ? 0.0f : 1.0f;
    _progressView.frame = progressFrame;
}
