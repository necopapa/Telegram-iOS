...
- (void)setSelected:(BOOL)selected
{
    ...
    _topLeftView.highlighted = selected;
    _topRightView.highlighted = selected;
    _bottomLeftView.highlighted = selected;
    _bottomRightView.highlighted = selected;
...
- (void)setButtonsAndActions:(NSArray *)buttonsAndActions watcherHandle:(ASHandle *)watcherHandle
{
    ...
    for (NSDictionary *dict in buttonsAndActions)
    {
        ...
        else
        {
            ...
            [buttonView setTitleShadowColor:UIColorRGBA(0x000000, 0.8f) forState:UIControlStateNormal];
            [buttonView setTitleShadowColor:UIColorRGBA(0x186bcb, 0.6f) forState:UIControlStateHighlighted];
            [buttonView setTitleShadowColor:UIColorRGBA(0x186bcb, 0.6f) forState:UIControlStateSelected];
            [buttonView setTitleShadowColor:UIColorRGBA(0x186bcb, 0.6f) forState:UIControlStateHighlighted | UIControlStateSelected];
            buttonView.titleLabel.shadowOffset = CGSizeMake(0, -1);
            buttonView.titleLabel.font = [UIFont boldSystemFontOfSize:14];
...
- (void)showInView:(UIView *)view fromRect:(CGRect)rect
{
    ...
    [UIView animateWithDuration:0.04 delay:0 options:UIViewAnimationOptionCurveEaseOut | UIViewAnimationOptionBeginFromCurrentState animations:^
    {
        //self.alpha = 1.0f;
    } completion:nil];
...
- (void)layoutSubviews
{
    ...
    for (TGMenuButtonView *buttonView in _buttonViews)
    {
        ...
        {
            buttonView.topLeftView.frame = CGRectMake(linePosition, 0, lineWidth, buttonView.topLeftView.frame.size.height);
            buttonView.topRightView.frame = CGRectMake(linePosition, 0, 0, buttonView.topLeftView.frame.size.height);
        }
        else
        {
            buttonView.topLeftView.frame = CGRectMake(linePosition, 0, MAX(0, _arrowTopView.frame.origin.x - buttonView.frame.origin.x - linePosition), buttonView.topLeftView.frame.size.height);
            
            float secondLinePosition = MIN(linePosition + lineWidth, buttonView.topLeftView.frame.origin.x + buttonView.topLeftView.frame.size.width + _arrowTopView.frame.size.width);
            buttonView.topRightView.frame = CGRectMake(secondLinePosition, 0, MAX(0, linePosition + lineWidth - secondLinePosition), buttonView.topLeftView.frame.size.height);
        }
        
        if (_arrowOnTop || !containsArrow)
        {
            buttonView.bottomLeftView.frame = CGRectMake(linePosition, 41 - 4, lineWidth, buttonView.bottomLeftView.frame.size.height);
            buttonView.bottomRightView.frame = CGRectMake(linePosition, 41 - 4, 0, buttonView.bottomLeftView.frame.size.height);
        }
        else
        {
            buttonView.bottomLeftView.frame = CGRectMake(linePosition, 41 - 4, MAX(0, _arrowTopView.frame.origin.x - buttonView.frame.origin.x - linePosition), buttonView.bottomLeftView.frame.size.height);
            
            float secondLinePosition = MIN(linePosition + lineWidth, buttonView.topLeftView.frame.origin.x + buttonView.bottomLeftView.frame.size.width + _arrowTopView.frame.size.width);
            buttonView.bottomRightView.frame = CGRectMake(secondLinePosition, 41 - 4, MAX(0, linePosition + lineWidth - secondLinePosition), buttonView.bottomLeftView.frame.size.height);
        }
    }
    ...
    for (TGMenuButtonView *listButtonView in _buttonViews)
    {
        ...
        if (listButtonView == buttonView)
        {
            ...
            if (index < _buttonDescriptions.count)
            {
                ...
                [_watcherHandle requestAction:@"menuAction" options:action];
            }
