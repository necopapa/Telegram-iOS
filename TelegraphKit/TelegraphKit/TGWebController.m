...
- (UILabel *)_createCustomTitleLabel
{
    ...
    titleLabel.shadowColor = [TGViewController titleShadowColorForStyle:self.style];
    titleLabel.shadowOffset = [TGViewController titleShadowOffsetForStyle:self.style];
...
- (void)webViewDidFinishLoad:(UIWebView *)__unused webView
{
    ...
    bool fadeIn = self.titleText.length == 0 || [self.titleText isEqualToString:@" "];
    [self setTitleText:pageTitle];
    if (fadeIn)
        [self fadeInTitleText];
...
- (void)webView:(UIWebView *)__unused webView didFailLoadWithError:(NSError *)__unused error
{
    ...
    if (![self.titleText isEqualToString:pageTitle])
        self.titleText = pageTitle;
    
    if (error.code == -999)
        return;
    
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:TGLocalized(@"Web.Error") delegate:nil cancelButtonTitle:NSLocalizedString(@"Common.OK", nil) otherButtonTitles:nil];
    [alertView show];
