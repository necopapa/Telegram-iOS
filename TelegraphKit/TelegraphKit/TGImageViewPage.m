...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        _controlsContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 20 + 44 + 6, 320, 29)];
        _controlsContainer.alpha = 0.0f;
        _controlsContainer.tag = ((int)0x6FC81BDB);
        [self addSubview:_controlsContainer];
        
        UIImage *controlsContainerBackgroundImage = [UIImage imageNamed:@"VideoScrubberBackground.png"];
        
        UIImageView *controlsContainerBackground = [[UIImageView alloc] initWithImage:[controlsContainerBackgroundImage stretchableImageWithLeftCapWidth:(int)(controlsContainerBackgroundImage.size.width / 2) topCapHeight:0]];
        controlsContainerBackground.frame = CGRectMake(5, 0, _controlsContainer.frame.size.width - 10, 29);
        controlsContainerBackground.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        [_controlsContainer addSubview:controlsContainerBackground];
        ...
        _disabledSliderView.frame = CGRectMake(48, 9 + (TGIsRetina() ? 0.5f : 0.0f), _controlsContainer.frame.size.width - 48 * 2, disabledTrackImage.size.height);
        ...
        _sliderView.frame = CGRectMake(46, 2 + (TGIsRetina() ? 0.5f : 0.0f), _controlsContainer.frame.size.width - 92, _sliderView.frame.size.height);
        ...
        _forwardTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 42, 28)];
        _forwardTimeLabel.backgroundColor = [UIColor clearColor];
        _forwardTimeLabel.textAlignment = UITextAlignmentRight;
        _forwardTimeLabel.font = [UIFont boldSystemFontOfSize:12];
        _forwardTimeLabel.textColor = [UIColor whiteColor];
        _forwardTimeLabel.shadowColor = UIColorRGBA(0x000000, 0.3f);
        _forwardTimeLabel.shadowOffset = CGSizeMake(0, 1);
        _forwardTimeLabel.text = @"-:--";
        [_controlsContainer addSubview:_forwardTimeLabel];
        
        _backwardTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(_controlsContainer.frame.size.width - 42, 0, 42, 28)];
        ...
        _backwardTimeLabel.font = [UIFont boldSystemFontOfSize:12];
        _backwardTimeLabel.textColor = [UIColor whiteColor];
        _backwardTimeLabel.shadowColor = UIColorRGBA(0x000000, 0.3f);
        _backwardTimeLabel.shadowOffset = CGSizeMake(0, 1);
...
- (void)setFrame:(CGRect)frame
{
    ...
    _controlsContainer.frame = CGRectMake(0, statusBarHeight + 44 + 6, frame.size.width, _controlsContainer.frame.size.height);
}
...
- (void)playbackStateChanged:(NSNotification *)__unused notification
{
    ...
    switch (_moviePlayer.playbackState)
    {
        ...
        {   
            [self playbackTimerEvent];
            _playbackTimer = [[NSTimer alloc] initWithFireDate:[[NSDate alloc] initWithTimeIntervalSinceNow:0.0] interval:0.5 target:self selector:@selector(playbackTimerEvent) userInfo:nil repeats:true];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _videoContainerView = [[TGVideoContainerView alloc] init];
        ...
        _topProgressViewContainer = [[UIView alloc] initWithFrame:CGRectMake(8, self.frame.size.height - 61, self.frame.size.width - 16, progressBackgroundImage.size.height)];
        ...
        _bottomProgressViewContainer = [[UIView alloc] initWithFrame:CGRectMake(8, self.frame.size.height - 17, self.frame.size.width - 16, progressBackgroundImage.size.height)];
...
- (void)loadItem:(id<TGMediaItem>)mediaItem placeholder:(UIImage *)placeholder willAnimateAppear:(bool)willAnimateAppear
{
    ...
    if ([mediaItem type] == TGMediaItemTypePhoto && [mediaItem imageInfo] != nil)
    {
        ...
        if (thumbnailImage == nil)
        {
            ...
            if (thumbnailImage == nil)
            {
                _currentThumbnailPath = [TGRemoteImageView preloadImage:thumbnailUrl filter:nil cache:[TGRemoteImageView sharedCache] allowThumbnailCache:false watcher:self];
            }
        }
...
- (void)animateDisappearToImage:(UIImage *)__unused toImage toView:(UIView *)toView aboveView:(UIView *)aboveView toRect:(CGRect)toRect toContainerImage:(UIImage *)toContainerImage toInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation keepAspect:(bool)keepAspect backgroundAlpha:(float)backgroundAlpha swipeVelocity:(float)swipeVelocity completion:(dispatch_block_t)completion
{
    ...
    [_transitionHelper beginTransitionOut:_imageView fromView:self toView:toView aboveView:aboveView interfaceOrientation:toInterfaceOrientation toRectInWindowSpace:toRect toImage:toContainerImage keepAspect:keepAspect swipeVelocity:swipeVelocity completion:completion];
}
...
- (void)updateControlsOffset:(float)offsetY
{
    ...
    frame.origin.y = _statusBarHeight + 44 + 6 - offsetY;
    _videoContainerView.controlsContainer.frame = frame;
    
    self.clipsToBounds = false;
    
    _topProgressViewContainer.frame = CGRectMake(8, self.frame.size.height - 61 - offsetY, self.frame.size.width - 16, _topProgressViewContainer.frame.size.height);
    _bottomProgressViewContainer.frame = CGRectMake(8, self.frame.size.height - 17 - offsetY, self.frame.size.width - 16, _bottomProgressViewContainer.frame.size.height);
}
