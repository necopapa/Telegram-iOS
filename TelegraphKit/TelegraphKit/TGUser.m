...
- (NSString *)firstName
{
    return (_phonebookFirstName.length != 0 || _phonebookLastName.length != 0) ? _phonebookFirstName : ((_firstName.length != 0 || _lastName.length != 0) ? _firstName : (_phoneNumber.length == 0 ? @"Unknown" : [self formattedPhoneNumber]));
}
...
- (NSString *)formattedPhoneNumber
{
    ...
    else
    {
        ...
        if (_phoneNumber.length != 0)
            cachedFormattedNumber = [TGStringUtils formatPhone:_phoneNumber forceInternational:true];
