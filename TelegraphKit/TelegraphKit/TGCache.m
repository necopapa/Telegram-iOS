...
- (void)freeMemoryCache:(NSUInteger)targetSize
{
    dispatch_block_t block = ^
    {
        if (_memoryCacheSize > targetSize)
        {
            __unused int sizeBefore = _memoryCacheSize;
            ...
            for (int i = 0; i < sortedKeys.count && _memoryCacheSize > targetSize; i++)
            {
                NSString *key = [sortedKeys objectAtIndex:i];
                TGCacheRecord *record = [_memoryCache objectForKey:key];
                _memoryCacheSize -= record.size;
                if (_memoryCacheSize < 0)
                    _memoryCacheSize = 0;
                [_memoryCache removeObjectForKey:key];
            }
            //TGLog(@"TGCache: freed %d kbytes (cache size: %d kbytes)", (int)((sizeBefore - _memoryCacheSize) / 1024), (int)(_memoryCacheSize / 1024));
        }
    };
    if ([NSThread isMainThread])
        block();
...
- (void)cacheImage:(UIImage *)image withData:(NSData *)data url:(NSString *)url availability:(int)availability
{
    if (image != nil && (availability & TGCacheMemory))
    {
        ...
        dispatch_block_t block = ^
        {
            ...
            if (cacheRecord != nil)
            {
                _memoryCacheSize -= cacheRecord.size;
                cacheRecord.date = CFAbsoluteTimeGetCurrent();
            ...
            if (_memoryCacheSize >= _imageMemoryLimit + _imageMemoryEvictionInterval)
    ...
    if ((data != nil || image != nil) && (availability & TGCacheDisk))
    {
