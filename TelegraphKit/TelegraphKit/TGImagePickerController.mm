...
void dispatchOnAssetsProcessingQueue(dispatch_block_t block)
{
    ...
    if (dispatch_get_specific != NULL)
        ...
        isCurrentQueueAssetsProcessingQueue = dispatch_get_current_queue() == assetsProcessingQueue();
...
+ (void)loadAsset:(NSString *)url completion:(void (^)(ALAsset *asset))completion
{
    dispatchOnAssetsProcessingQueue(^
    {
        if (sharedLibrary != nil)
        {
            [sharedLibrary assetForURL:[[NSURL alloc] initWithString:url] resultBlock:^(ALAsset *asset)
            {
                if (completion)
                    completion(asset);
            } failureBlock:^(__unused NSError *error)
...
- (void)loadView
{
    ...
    _assetsInRow = (int)(screenSize.width / (75 + 4));
    ...
    _tableView.contentInset = UIEdgeInsetsMake(20 + 42, 0, (_groupUrl == nil ? 4 : 4) + 48, 0);
    _tableView.scrollIndicatorInsets = UIEdgeInsetsMake(20, 0, 48, 0);
    ...
    if (self.navigationController.viewControllers.count > 1 && [self.navigationController.viewControllers objectAtIndex:0] != self)
    {
        ...
        TGToolbarButton *backButton = [[TGToolbarButton alloc] initWithCustomImages:imageNormal imageNormalHighlighted:imageNormalHighlighted imageLandscape:nil imageLandscapeHighlighted:nil textColor:[UIColor whiteColor] shadowColor:UIColorRGBA(0x000000, 0.3f)];
        backButton.backSemantics = true;
        backButton.paddingLeft = 15;
        backButton.paddingRight = 9;
        backButton.text = @"Albums";
        [backButton sizeToFit];
        [backButton addTarget:self action:@selector(performClose) forControlEvents:UIControlEventTouchUpInside];
        backButton.frame = CGRectOffset(backButton.frame, 4, 28);
    ...
    galleryNameLabel.text = _groupTitle == nil ? @"Camera Roll" : _groupTitle;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    int assetsInRow = (int)(tableFrame.size.width / (75 + 4));
    if (assetsInRow != _assetsInRow)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)
    {
        ...
        if (rowStartIndex < _assetList.count)
        {
            ...
            if (imageCell == nil)
            {
                imageCell = [[TGImagePickerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:imageCellIdentifier selectionControls:!_avatarSelectionMode imageSize:75];
                imageCell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
            
            [imageCell resetImages:_assetsInRow];
...
{
    _doneButton.enabled = _pagingScrollView.alpha > FLT_EPSILON || _selectedAssets.count != 0;
...
- (void)assetTapped:(TGImagePickerAsset *)asset imageCell:(TGImagePickerCell *)imageCell
{
    ...
    else
    {
        ...
        if (assetIndex >= 0)
        {
            ...
            {   
                TGImageViewPage *page = [[TGImageViewPage alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - _panelView.frame.size.height)];
                ...
                [page animateAppearFromImage:[[pageList objectAtIndex:assetIndex] immediateThumbnail] fromView:self.view aboveView:_tableView fromRect:fromRect toInterfaceOrientation:self.interfaceOrientation completion:^
                {
                    ...
                    float pageGap = 40;
                    _pagingScrollView = [[TGImagePagingScrollView alloc] initWithFrame:CGRectMake(-pageGap / 2, 0, self.view.bounds.size.width + pageGap, self.view.bounds.size.height - _panelView.frame.size.height)];
...
- (void)dismissPagingScrollView:(float)swipeVelocity
{
    ...
    if (CGRectIsEmpty(targetRect))
    {
        ...
        [page animateDisappearToImage:nil toView:self.view aboveView:_tableView toRect:targetRect toContainerImage:nil toInterfaceOrientation:self.interfaceOrientation keepAspect:true backgroundAlpha:_backgroundView.alpha swipeVelocity:swipeVelocity completion:^
         {
             [_pagingScrollView removeFromSuperview];
             _pagingScrollView.delegate = nil;
             _pagingScrollView = nil;
    ...
    else
    {
        ...
        [page animateDisappearToImage:thumbnail toView:self.view aboveView:_tableView toRect:targetRect toContainerImage:thumbnail toInterfaceOrientation:self.interfaceOrientation keepAspect:true backgroundAlpha:_backgroundView.alpha swipeVelocity:swipeVelocity completion:^
        {
            [_pagingScrollView removeFromSuperview];
            _pagingScrollView.delegate = nil;
            _pagingScrollView = nil;
    ...
    if (key != nil)
    {
        ...
        if (isSelected)
        {
            ...
            if (currentImage != nil)
            {
                ...
                [self.view insertSubview:containerView belowSubview:_checkButton];
