...
- (void)resetImages:(int)imagesInRow
{
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < limit; i++)
    {
        ...
        {
            if (_selectionControls)
                buttonView.frame = CGRectMake(currentX + imageSize.width - 33, 5, 33, 33);
...
static UITableView *findTableView(UIView *view)
{
    if (view == nil)
        return nil;
    ...
    return findTableView(view.superview);
}
...
- (void)checkButtonPressed:(id)button
{
    ...
    for (int i = 0; i < (int)_assets.count; i++)
    {
        ...
        {
            UITableView *superview = findTableView(self.superview);
