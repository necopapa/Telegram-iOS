...
- (int)preferredPartSize
{
    ...
    if (_takenTimeSamples < 4)
        return 128 * 1024;
    
    if (_averageDownloadSpeed > 120 * 1024)
        return 512 * 1024;
    else if (_averageDownloadSpeed > 80 * 1024)
        return 256 * 1024;
...
- (void)downloadFileParts
{
    ...
    else
    {
        ...
        for (std::vector<std::pair<int, int> >::iterator it = requestList.begin(); it != requestList.end(); it++)
        {
            ...
            {
                token = [TGTelegraphInstance doDownloadVideoPart:_datacenterId videoId:_videoId accessHash:_accessHash offset:it->first length:it->second actor:self];
            }
            ...
            for (std::vector<int>::iterator it = sortedDownloadingParts.begin(); it != sortedDownloadingParts.end(); it++)
            {
                if (*it == nextWriteOffset)
                {
                    ...
                    if (_encryptionKey != nil && _runningEncryptionIv != nil)
                    {
                        ...
                        encryptWithAESInplaceAndModifyIv(decryptedData, _encryptionKey, _runningEncryptionIv, false);
