...
- (void)loadView
{
    ...
    self.titleText = NSLocalizedString(@"ConversationMedia.Title", nil);
    ...
    _imagesPerRow = (int)([TGViewController screenSizeForInterfaceOrientation:self.interfaceOrientation].width / (75 + 4));
...
{
    _imagesPerRow = (int)([TGViewController screenSizeForInterfaceOrientation:toInterfaceOrientation].width / (75 + 4));
...
- (void)updatePlaceholder
{
    if (!_canLoadMore && _presentationListModel.count == 0)
    {
        ...
        {
            UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"PhotosBlankPlaceholder.png"]];
            
            UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 320, 320)];
            labelView.text = TGLocalized(@"ConversationMedia.Empty");
            labelView.font = [UIFont boldSystemFontOfSize:17];
            labelView.textColor = UIColorRGB(0x808895);
            labelView.backgroundColor = [UIColor whiteColor];
            [labelView sizeToFit];
            
            CGRect containerFrame = CGRectMake(0, 0, MAX(imageView.frame.size.width, labelView.frame.size.width), labelView.frame.size.height + imageView.frame.size.height + 12 + 36);
            ...
            [_placeholderContainerView addSubview:imageView];
            [_placeholderContainerView addSubview:labelView];
            
            imageView.frame = CGRectIntegral(CGRectOffset(imageView.frame, (_placeholderContainerView.frame.size.width - imageView.frame.size.width) / 2, 0));
            labelView.frame = CGRectIntegral(CGRectOffset(labelView.frame, (_placeholderContainerView.frame.size.width - labelView.frame.size.width) / 2, imageView.frame.size.height + 12));
        }
