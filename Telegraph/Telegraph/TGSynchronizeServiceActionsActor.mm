            ...
            if (futureActions.count != 0)
            {
                ...
                else if ([action isKindOfClass:[TGSynchronizeEncryptedChatSettingsFutureAction class]])
                {
                    ...
                    else
                    {
                        ...
                        self.cancelToken = [TGTelegraphInstance doSendEncryptedServiceMessage:settingsAction.uniqueId accessHash:accessHash randomId:settingsAction.messageRandomId data:[TGConversationSendMessageActor encryptMessage:serializedData key:key keyId:keyId] actor:self];
                    }
