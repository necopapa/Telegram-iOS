...
- (void)clearColorMapping
{
    ...
    [TGConversationMessageItemView clearColorMapping];
}
...
- (UIColor *)userColor:(int)uid
{
    ...
    dispatch_once(&onceToken, ^
    {
        userColors[0] = UIColorRGB(0xee4928);
        userColors[1] = UIColorRGB(0x41a903);
        userColors[2] = UIColorRGB(0xe09602);
        userColors[3] = UIColorRGB(0x0f94ed);
        userColors[4] = UIColorRGB(0x8f3bf7);
        userColors[5] = UIColorRGB(0xfc4380);
        userColors[6] = UIColorRGB(0x00a1c4);
        userColors[7] = UIColorRGB(0xeb7002);

    });
    
    return userColors[colorIndexForUid(uid) % 8];
}
...
+ (UIImage *)mediaListImageShadow
{
    static UIImage *image = nil;
    if (image == nil)
        TGStretchableImageInCenterWithName(@"MediaListImageShadow.png", image);
    return image;
}
