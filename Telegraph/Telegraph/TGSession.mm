...
- (bool)isCurrentQueueNetworkQueue
{
    ...
    else
    {
        ...
        if (queue == [self networkDispatchQueue])
            return true;
    }
...
- (void)setTimeDifference:(int)timeDifference
{
    ...
    [TGDatabaseInstance() setTimeDifferenceFromUTC:_timeOffsetFromUTC];
    //TGLog(@"===== Time difference: %d, ping time: %f", _timeDifference, _currentPingTime);
    if (store)
    {
        [self storeSession];
...
- (void)loadSession
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        if (ret != nil && [ret isKindOfClass:[NSDictionary class]])
        {
            ...
            if ((datacenterSetId == TGAppDelegateInstance.useDifferentBackend? 1 : 0) && [[dict objectForKey:@"version"] intValue] == 5 && keychainKeyMatches)
            {
                _sessionsToDestroy.clear();
        ...
        {
            if (TGAppDelegateInstance.useDifferentBackend)
            {
...
- (void)storeSession
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        [data setObject:[NSNumber numberWithInt:TGAppDelegateInstance.useDifferentBackend ? 1 : 0] forKey:@"backendId"];
...
#warning test
    ...
    dispatch_async(dispatch_get_main_queue(), ^
    {
        [[AFNetworkActivityIndicatorManager sharedManager] setActivityCount:hasActiveRequests ? 1 : 0];
    });
    ...
    {
        if (*it != _currentDatacenterId && *it != _movingToDatacenterId && TGTelegraphInstance.clientUserId != 0 && _unavailableDatacenterIds.find(*it) == _unavailableDatacenterIds.end())
        {
...
//#endif
            ...
            NSArray *statelessUpdates = [TGApplyUpdatesActor filterStatelessUpdates:(TLUpdates *)message];
            ...
            {
                NSArray *convertedUpdates = [TGApplyUpdatesActor makeStatelessUpdates:(TLUpdates *)message];
...
- (void)authorizeOnMovingDatacenter
{
    ...
    if (_movingAuthorization != nil)
    {
        ...
        importAuthorization.n_id = TGTelegraphInstance.clientUserId;
