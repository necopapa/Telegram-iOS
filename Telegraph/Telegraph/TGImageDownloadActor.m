...
- (void)execute:(NSDictionary *)options
{
    ...
    if ([url hasPrefix:@"asset-original:"] || [url hasPrefix:@"asset-thumbnail:"])
    {
        ...
        [TGImagePickerController loadAsset:assetUrl completion:^(ALAsset *asset)
        {
            UIImage *image = nil;
            if (asset != nil)
            {
    ...
    if ([url hasPrefix:@"local://wallpaper"])
    {
        NSString *imageUrl = [url substringFromIndex:[url rangeOfString:@"://"].location + 3];
        ...
        else
        {
            ...
            UIImage *rawImage = [[UIImage alloc] initWithContentsOfFile:filePath];
            if (processor != nil)
                image = processor(rawImage);
            else
                image = [rawImage preloadedImage];
        ...
        [ActionStageInstance() dispatchOnStageQueue:^
        {
            ...
            else
            {
                ...
                if ([self.path hasPrefix:@"/img/(download:"])
                {
                    ...
                    self.downloadCompletionBlock = ^(NSData *data)
                    {
                        if (data != nil)
                        {
                                    ...
                                    if (image != nil)
                                    {
                                        ...
                                        [ActionStageInstance() dispatchOnStageQueue:^
                                        {
                                            ...
                                            if (imageInfo != nil && imageForThumbnail != nil && ![url hasPrefix:@"upload"])
                                            {
                                                ...
                                                if (thumbnailUrl != nil)
                                                {
                                                    thumbnailSize = TGFitSize(CGSizeMake(imageForThumbnail.size.width * imageForThumbnail.scale, imageForThumbnail.size.height * imageForThumbnail.scale), [TGConversationController preferredInlineThumbnailSize]);
