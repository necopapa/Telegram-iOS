    ...
    if (buttonIndex != actionSheet.cancelButtonIndex)
    {
        ...
        else if (buttonIndex == 1 || buttonIndex == 2)
        {
            ...
            if (iosMajorVersion() <= 5)
            {
                ...
                CGSize screenSize = [TGViewController screenSizeForInterfaceOrientation:imageViewController.interfaceOrientation];
                navigationController.view.frame = CGRectMake(0, screenSize.height, screenSize.width, screenSize.height);
                [imageViewController.view addSubview:navigationController.view];
                ...
                } completion:^(BOOL finished)
                {
                    ...
                    if (finished)
                    {
                        ...
                        [imageViewController presentViewController:navigationController animated:false completion:nil];
...
{
    TGProfileController *profileController = nil;
    
    for (UIViewController *viewController in [TGAppDelegateInstance.mainNavigationController.viewControllers reverseObjectEnumerator])
    {
        if ([viewController isKindOfClass:[TGProfileController class]] && ((TGProfileController *)viewController).uid == TGTelegraphInstance.clientUserId)
        {
            profileController = (TGProfileController *)viewController;
            break;
        }
    }
    ...
    {
        for (UIViewController *viewController in TGAppDelegateInstance.mainTabsController.viewControllers)
        {
            if ([viewController isKindOfClass:[TGProfileController class]] && ((TGProfileController *)viewController).uid == TGTelegraphInstance.clientUserId)
            {
                profileController = (TGProfileController *)viewController;
    ...
    if (profileController != nil)
    {
        [profileController _updateProfileImage:image];
