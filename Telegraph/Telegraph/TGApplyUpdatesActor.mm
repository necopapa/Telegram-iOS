...
- (void)failSequence
{
    ...
    [TGSession instance].stateConsistencyFails++;
...
- (void)processUpdates:(NSArray *)updatesArray stateSeq:(int)stateSeq completeAction:(bool)completeAction
{
    ...
    for (TGUpdate *updateSet in updatesArray)
    {
        ...
        for (TLUpdate *update in updateSet.updates)
        {
            ...
            else if ([update isKindOfClass:updateNewEncryptedMessageClass])
            {
                ...
                if (updateSet.messageDate != 0 && updateSet.messageDate - date < 4 && updateNewEncryptedMessage.message != nil && stateQts != 0)
    ...
    if (!failedProcessing)
    {
        ...
        [TGUpdateStateRequestBuilder applyUpdates:addedMessages addedParsedMessages:nil otherUpdates:allUpdates usersDesc:usersToProcess chatsDesc:chatsToProcess chatParticipantsDesc:nil];
...
#warning optimize
                        ...
                        if (soundId == 0)
                            soundId = INT_MAX;
                        else if (soundId == 1 || soundId == 2)
                            soundId = 0;
                        
                        if (soundId != INT_MAX)
                            localNotification.soundName = soundId == 1 ? UILocalNotificationDefaultSoundName : [[NSString alloc] initWithFormat:@"%d.m4a", soundId];
                        
                        if (!attachmentFound)
                        {
                            if (message.cid <= INT_MIN)
                            {
                                text = TGLocalized(@"ENCRYPTED_MESSAGE");
                            }
                            else if (message.cid > 0)
                            {
                                if (globalMessagePreviewText)
